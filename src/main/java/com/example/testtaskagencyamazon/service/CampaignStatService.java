...
public class CampaignStatService {
  ...
  public Map<Long, SPCampaignStatistic> getSPCampaignsStatistics(
    ...
    for (SPCampaignReport report : reports) {
      ...
      final SPCampaignReport report_;
      SPCampaignStatistic statistic = campaignAnalyticMap.get(campaignId);
      if (statistic != null) {
        report_ = getReport(report, statistic);
      } else {
        report_ = report;
      }

      campaignAnalyticMap.computeIfAbsent(campaignId, k -> new SPCampaignStatistic(report_))
          .add(new SPCampaignStatistic(report_));
    }
  ...
  private SPCampaignReport getReport(SPCampaignReport report, SPCampaignStatistic statistic) {
    SPCampaignReport report_ = new SPCampaignReport();
    report_.setClicks(report.getClicks() + statistic.getClicks());
    report_.setImpressions(report.getImpressions() + statistic.getImpressions());
    report_.setReportId(report.getReportId());
    report_.setProfileId(report.getProfileId());
    report_.setPortfolioId(report.getPortfolioId());
    report_.setCampaignId(report.getCampaignId());
    report_.setCampaignName(report.getCampaignName());
    report_.setCampaignStatus(report.getCampaignStatus());
    report_.setCost(report.getCost().add(statistic.getCost()).round(ROUND));
    report_.setCostPerClick(report.getCostPerClick().add(statistic.getCostPerClick()).round(ROUND));
    report_.setDate(report.getDate());
    report_.setPurchases1d(report.getPurchases1d());
    report_.setPurchases14d(report.getPurchases14d() + statistic.getPurchases());
    report_.setPurchasesSameSku1d(report.getPurchasesSameSku1d());
    report_.setPurchasesSameSku14d(report.getPurchasesSameSku14d());
    report_.setSales1d(report.getSales1d());
    report_.setSales14d(report.getSales14d());
    report_.setSpend(report.getSpend());
